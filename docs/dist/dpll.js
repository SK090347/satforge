...
function simplify(clauses, assign) {
    const out = [];
...
function unitPropagate(clauses, assign, stats) {
    let cur = clauses;
    let changed = true;
    while (changed && cur) {
        changed = false;
        for (const c of cur) {
            ...
            const v = Math.abs(lit);
            const want = lit > 0;
            if (assign.has(v) && assign.get(v) !== want)
                return null;
            if (!assign.has(v)) {
                assign.set(v, want);
                stats.propagations++;
        ...
        if (changed)
            cur = simplify(cur, assign);
    }
    return cur;
}
function pickVar(clauses, assign) {
...
function dpll(clauses, assign, stats) {
